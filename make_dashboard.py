...
.wbCaption{font-size:11.5px;color:var(--ink-faint);margin-top:12px;line-height:1.5;
...
function renderResults(wh, orders, results, label, locName, ordName){
  ...
  // A real render (orders + a warehouse, routes and all) always wins over
  // whatever Section 2's floor-only preview last left showing.
  document.getElementById('wbCaption').hidden = true;
...
// Draws just the floor - racking, cross-aisles, staging, aisle numbers - into
// the SAME map the rest of the page uses, with no routes and no results
// table. There is only one map on this page; Section 2 borrows it rather
// than keeping a second, separate one that would drift out of sync with
// whatever Section 1 is currently showing.
function renderFloorOnly(wh, captionText){
  var plan = buildFloorPlan(wh);
  var map = document.getElementById('map');
  map.setAttribute('viewBox', '0 0 ' + Math.round(plan.width) + ' ' + Math.round(plan.height));
  map.innerHTML = svgFloor(plan);

  document.getElementById('tag').textContent = 'Warehouse preview';
  document.getElementById('srcline').textContent =
    wh.pickSlotCount() + ' slots · ' + wh.aisleCount() + ' aisles · no orders loaded against this shape yet';
  ['stat-was', 'stat-now', 'stat-removed'].forEach(function(id){ document.getElementById(id).textContent = '—'; });
  document.getElementById('stat-improvement').textContent = '—';
  document.getElementById('spreadline').textContent =
    'Floor plan only - drop an orders file into Section 1 above (or click Reset to sample data there) to see routes and figures.';
  document.getElementById('cap').textContent = '';
  document.getElementById('tbody').innerHTML =
    '<tr><td colspan="8" style="color:var(--ink-faint);padding:16px 12px">No orders loaded against this warehouse yet.</td></tr>';
  document.getElementById('shown').textContent = '0';
  document.getElementById('total-count').textContent = '0';

  document.getElementById('wbCaption').textContent = captionText;
  document.getElementById('wbCaption').hidden = false;
}
...
<div class="panel upload warehouseBuilder">
  ...
  <div class="uploadbody">
    ...
    <p class="wbCaption" id="wbCaption" hidden></p>
  </div>
</div>
